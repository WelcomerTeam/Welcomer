    ...
    <table class="min-w-full border-spacing-2">
      ...
      <tbody class="divide-y divide-gray-200 dark:divide-secondary-light">
        <tr v-for="(reactionRole, index) in this.$props.modelValue.reaction_roles" :key="index">
          ...
          <td class="py-3">
            <discord-embed class="flex-1" :embeds="reactionRole.is_system_message ? parseDict(reactionRole.message)?.embeds : []" :content="reactionRole.is_system_message ? parseDict(reactionRole.message)?.content : ''" :buttons="getReactionRoleButtons(reactionRole.type, reactionRole.roles)" :isLight="true" :showAuthor="false" />
          </td>
